...
export function FilterSidebar() {
  ...
  const { filters, setColorFilter, setSizeFilter, setPriceRangeFilter, resetFilters } = useFilter()
  ...
  const handleMinPriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(value)) {
      setPriceRangeFilter([value, filters.priceRange[1]])
    }
  }
  ...
  const handleMaxPriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = Number.parseFloat(e.target.value)
    if (!isNaN(value)) {
      setPriceRangeFilter([filters.priceRange[0], value])
    }
  }
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {colors.map((color) => (
                <button
                  ...
                  className={`flex flex-col items-center ${(filters.colors || []).includes(color) ? "ring-2 ring-black ring-offset-1" : ""}`}
